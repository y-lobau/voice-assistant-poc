...
export class BlinktController implements IVisualFeedback{
	...
	private clearIntervals(): void {
	  ...
	  private startInterval(intervalFunction: () => void, delay: number): NodeJS.Timeout {
	  ...
	  private handleEvent(start: boolean, color: [number, number, number], initialBrightness: number, isPulsing: boolean, isSequential: boolean = false, isBackAndForth: boolean = false): void {
		if (!start) {
		  this.clearIntervals();
		  return;
		}
